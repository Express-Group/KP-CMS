 ...
 <?php if(isset($section_mapping)) { 
				 ...
				 if($this->uri->segment(2) == 'video_manager') {
					 if(folder_name == 'niecpan')
						$condition = $mapping['Sectionname'] == 'ವಿಡಿಯೋ' ;
					else 
		...
		if(parseInt(Object.keys(publish_object).length) != 0) {
			...
			if(publish_status == true) {
				...
				$.ajax({ 
					...
					type:"POST",
					data:{"contentids" : content_data,"content_type" : content_type},
...
$("#flash_msg_id").slideDown(function() {
    ...
    $('#example').dataTable( {
		...
		"ajax": {
            "url": "<?php echo base_url().folder_name; ?>/audio_video_manager/audio_video_datatable",
			"type" : "POST",
			"data" : {
		...
		clear_all_checkbox();
			...
			$("#search_based_date").change(function()
			{
			 ...
			 $(document.body).on('click', '#status_change', function(event)
			 {
				...
				type:"POST",
				data:{"contentid":id, "status":final_status,"content_type":content_type},
				success: function(data) 
					{
						if(data == 'success')
						{
