...
.article_image1,.ArticleImageContainer1{width:22%;}
.article_image .set_image{margin-left: 3px;}
.article_image a{font-size: 11px;}
.article_image .del_image{padding: 10px 11px;}
</style>
...
<textarea class="ckeditor"  id="article_head_line_id" name="txtArticleHeadLine"><?php echo set_value('txtArticleHeadLine'); ?><?php  if(isset($get_article_details['title'])) echo $get_article_details['title']; ?><?php  if(isset($_POST['content_title'])) echo $_POST['content_title']; ?></textarea>
            ...
            <input type="text" class="box-shad1 full-width" id="txtUrlTitle" name="txtUrlTitle"  	<?php if(isset($get_article_details['url_title'])) { echo "readonly"; } ?>  value="<?php if(isset($get_article_details['url_title'])) { echo htmlentities($get_article_details['url_title']); } echo set_value('txtUrlTitle'); ?> <?php  if(isset($_POST['content_title'])) echo htmlentities($_POST['content_title']); ?> " />
...
<option <?php if((strtolower($_POST['content_sectionname']) == strtolower(strip_tags($mapping['Sectionname']))) && strtolower($_POST['content_sectionname'])!='news'){ echo " selected ";} ?> id="MainSectionOption" style="color:#933;font-size:18px;"<?php /* if($mapping['Section_landing'] == 1 && $mapping['Sectionname'] != 'Columns' && $mapping['Sectionname'] != 'Magazine' && $mapping['Sectionname'] != 'The Sunday Standard' && $mapping['Sectionname'] != 'Editorials' ) { ?> disabled='disabled' <?php } */ ?>  class="blog_option" <?php if(set_value("ddMainSection") == $mapping['Section_id'] || (isset($get_article_details['content_id']) && $get_article_details['Section_id'] == $mapping['Section_id']  ) || ($page_name != 'edit_article' && $this->session->userdata('main_section') == $mapping['Section_id'] && folder_name == 'dmcpan')) { echo  "selected"; $section_bool=false; } ?> sectoin_data="<?php echo @$mapping['Sectionname']; ?>" rel="<?php echo @$mapping['LinkedToColumnist']; ?>"  value="<?php echo $mapping['Section_id']; ?>" url_structure="<?php echo mb_ucwords(str_replace("-"," ",str_replace("/"," > ",trim(@$mapping['BreadCrumb'])))); ?>" author_name="<?php echo (isset($mapping['AuthorID']) && $mapping['AuthorID'] != '')? get_authorname_by_id($mapping['AuthorID']) : '' ; ?>"><?php echo strip_tags($mapping['Sectionname']); ?></option>
  <?php if(!(empty($mapping['sub_section'])) ) { ?>
 
  <?php foreach($mapping['sub_section'] as $key=>$sub_mapping) { ?>
  
  <option <?php if((strtolower($_POST['content_sectionname']) == strtolower(strip_tags($mapping['Sectionname']))) && strtolower($_POST['content_sectionname'])!='news'){ echo " selected ";} ?>  id="MainSectionOption" <?php if(set_value("ddMainSection") == $sub_mapping['Section_id']  || (isset($get_article_details['content_id']) && $get_article_details['Section_id'] == $sub_mapping['Section_id'] ) || ($page_name != 'edit_article' && $this->session->userdata('main_section') == $sub_mapping['Section_id'] && folder_name == 'dmcpan')) { echo  "selected";  $section_bool=false; } ?>  sectoin_data="<?php echo @$mapping['Sectionname']; ?>"  rel="<?php echo @$sub_mapping['LinkedToColumnist']; ?>" value="<?php echo $sub_mapping['Section_id']; ?>" url_structure="<?php echo mb_ucwords(str_replace("-"," ",str_replace("/"," > ",trim(@$sub_mapping['BreadCrumb'])))); ?>" author_name="<?php echo (isset($sub_mapping['AuthorID']) && $sub_mapping['AuthorID'] != '')? get_authorname_by_id($sub_mapping['AuthorID']) : '' ; ?>">&nbsp;&nbsp;&nbsp;&nbsp;<?php echo strip_tags($sub_mapping['Sectionname']); ?></option>
		
		 <?php 
		 if(!(empty($sub_mapping['sub_sub_section']))) { ?>
		 
		   <?php foreach($sub_mapping['sub_sub_section'] as $sub_sub_mapping) { ?>
    <option <?php if((strtolower($_POST['content_sectionname']) == strtolower(strip_tags($mapping['Sectionname']))) && strtolower($_POST['content_sectionname'])!='news'){ echo " selected ";} ?> id="MainSectionOption" <?php if($sub_sub_mapping['Section_landing'] == 1) { ?> disabled='disabled' <?php } ?>  <?php if(set_value("ddMainSection") == $sub_sub_mapping['Section_id']  || (isset($get_article_details['content_id']) && $get_article_details['Section_id'] == $sub_sub_mapping['Section_id'] ) || ($page_name != 'edit_article' && $this->session->userdata('main_section') == $sub_sub_mapping['Section_id'] && folder_name == 'dmcpan'))  { echo  "selected";  $section_bool=false; }  ?>  rel="<?php echo @$sub_sub_mapping['LinkedToColumnist']; ?>" value="<?php echo $sub_sub_mapping['Section_id']; ?>"  sectoin_data="<?php echo @$sub_mapping['Sectionname']; ?>" url_structure="<?php echo mb_ucwords(str_replace("-"," ",str_replace("/"," > ",trim(@$sub_sub_mapping['BreadCrumb'])))); ?>" author_name="<?php echo (isset($sub_sub_mapping['AuthorID']) && $sub_sub_mapping['AuthorID'] == '')? get_authorname_by_id($sub_sub_mapping['AuthorID']) : '' ; ?>" >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo strip_tags($sub_sub_mapping['Sectionname']); ?></option>
		  <?php
 ...
 <?php echo form_error('ddMainSection'); ?>
  ...
  &nbsp;&nbsp;<input type="checkbox" name="todayspaper" id="todayspaper" class="HeightAuto" <?php if($page_name == 'edit_article' && isset($todayspaper) && $todayspaper > 0){ echo "checked";} ?>> Today's Paper
...
<textarea class="ckeditor" id="body_text" name="txtBodyText"><?php if(isset($get_article_details['ArticlePageContentHTML']) && @$_POST['utm_value']=='') echo $get_article_details['ArticlePageContentHTML']; echo set_value('txtBodyText'); ?><?php if(isset($_POST['utm_value'])){ echo @$_POST['utm_value']; } ?></textarea>
 <?php echo form_error('txtBodyText'); ?>
 <span id="article_body_error"></span>
 <input type="hidden" id="utm_folder_path" name="utm_folder_path" value="<?php echo @$_POST['utm_folder_path']; ?>">
 <input type="hidden" id="utm_file_path" name="utm_file_path" value="<?php echo @$_POST['utm_file_path']; ?>">
 <input type="hidden" id="utm_type" name="utm_type" value="<?php echo @$_POST['utm']; ?>">
 <input type="hidden" id="utm_desk_id" name="utm_desk_id" value="<?php echo @$_POST['utm_id']; ?>">
 <input type="hidden" id="utm_userid" name="utm_userid" value="<?php echo $this->session->userdata('userID'); ?>">
 <input type="hidden" id="utm_firstname" name="utm_firstname" value="<?php echo $this->session->userdata('first_name'); ?>">
 <input type="hidden" id="utm_lastname" name="utm_lastname" value="<?php echo $this->session->userdata('last_name'); ?>">
            </div>
           ...
           <a id="delete_article_image" rel="home" class="del_image" href="javascript:void(0);"  <?php if((isset($get_article_details['homepageimageid']['temp_id']) && $get_article_details['homepageimageid']['temp_id'] != '') ) { ?>  <?php } else {  ?>   style="display:none;" <?php  } ?> ><i class="fa fa-trash-o"></i></a> 
            ...
            <p id="article_uploaded_image"><?php if((set_value('imgArticleImageId') != '') || (isset($get_article_details['articleimageid']['temp_id']) && $get_article_details['articleimageid']['temp_id'] != '') || @$_POST['content_image']!='') echo "Image set"; else  echo "Image not set";  ?></p>
            
            <a class="set_image <?php if((set_value('imgArticleImageId') != '') || (isset($get_article_details['Articleimageid']) && $get_article_details['Articleimageid'] != '') || @$_POST['content_image']!='') echo " "; else  echo "BorderRadius3";  ?>" data-remodal-target="modal1" href="#"   id="article_image_set" onClick="ChangePopup('article')"><?php if((isset($get_article_details['articleimageid']['temp_id']) && $get_article_details['articleimageid']['temp_id'] != '') || @$_POST['content_image']!='' ) echo "Change Image"; else  echo "Set Image"; ?></a>
            
			 <a  id="edit_article_image" rel="article" class="del_image delbtn_border" href="javascript:void(0);" <?php if((isset($get_article_details['articleimageid']['temp_id']) && $get_article_details['articleimageid']['temp_id'] != '')  || @$_POST['content_image']!='') { ?>  <?php } else {  ?> style="display:none;"  <?php  } ?>><i class="fa fa-pencil"></i></a>
			
             <a  id="delete_article_image" rel="article" class="del_image" href="javascript:void(0);" <?php if((isset($get_article_details['articleimageid']['temp_id']) && $get_article_details['articleimageid']['temp_id'] != '') || @$_POST['content_image']!='' ) { ?>  <?php } else {  ?>   style="display:none;" <?php  } ?>><i class="fa fa-trash-o"></i></a>
             
             <input type="hidden" class="image-group"     rel="<?php echo @$get_article_details['articleimageid']['imagecontent_id']; ?> <?php if(@$utm_imgid!=''){ echo $utm_imgid; } ?>"   value="<?php if(isset($get_article_details['articleimageid']['temp_id'])) echo $get_article_details['articleimageid']['temp_id']; ?><?php  echo set_value('imgArticleImageId'); ?> <?php if(@$utm_t_image_id!=''){ echo $utm_t_image_id; } ?>" name="imgArticleImageId" id="article_image_gallery_id" />  
			...
			?>   
			<div class="article_image1 margin-right-0" style="margin-left: 25px;">
				<h3>Related Image /GFX</h3>
				<p id="relhome_uploaded_image"><?php echo "Image set"; ?></p>
				<a class="set_image" id="relhome_image_set"  data-remodal-target="modal5" href="#"  ><?php  echo "Set Image"; ?></a>
				<a style="display:none;" data-remodal-target="modal5"  id="reledit_article_image" rel="home" class="del_image delbtn_border" href="javascript:void(0);" ><i class="fa fa-pencil"></i></a>
				<a style="display:none;" id="reldelete_article_image" rel="home" class="del_image" href="javascript:void(0);"  ><i class="fa fa-trash-o"></i></a>              
            </div>				
       	 	</div>
            <div id="ArticleImageContainerId" class="ArticleImageContainer" <?php if($page_name == 'edit_article' && isset($get_article_details['homepageimageid']['temp_id']) == '' && isset($get_article_details['Sectionimageid']['temp_id']) == '' && isset($get_article_details['articleimageid']['temp_id']) == ''  ) { echo $display_none; }  if($page_name != 'edit_article' && $utm_imgid=='') {  echo $display_none;   } ?> >
        ...
        <p style="">Preview</p>
          ...
          <div class="ArticleImageContainer1 margin-right-0" id="article_image_container" <?php if(!isset($get_article_details['articleimageid']['temp_id']) &&  $utm_imgid=='') { ?> style="visibility:hidden;" <?php } ?>>
        <p style="">Preview</p>
          <img  id="article_image_src" src="<?php if(isset($get_article_details['articleimageid']['temp_id']) && $get_article_details['articleimageid']['temp_id'] != '') { echo $get_article_details['articleimageid']['temp_image'];  } ?> <?php if(@$utm_imgname!=''){ echo image_url.imagelibrary_image_path.$utm_imgname;} ?>"  />
		  <label>Caption :</label>
		   <input type="text" value="<?php echo htmlentities(@$get_article_details['articleimageid']['temp_caption']);   ?> <?php if(@$utm_imgcaption!=''){ echo htmlentities($utm_imgcaption); } ?>" name="article_image_caption"    class="margin-top-5 WidthFull" id="article_image_caption" />
		   <label>Image Alt :</label>
		   <input type="text" value="<?php echo htmlentities(@$get_article_details['articleimageid']['temp_alt']);   ?> <?php if(@$utm_imgcaption!=''){ echo htmlentities($utm_imgcaption); } ?>" name="article_image_alt"   class="margin-top-5 WidthFull"  id="article_image_alt" />
		   <?php
			if($utm_imgname!=''){
				$pos = strrpos($utm_imgname,'/');
				$artimagename = substr($utm_imgname,$pos + 1);
				$utm_artimg = explode('.',$artimagename);
			}
		   ?>
		   <label>Image Name :</label>
			<input type="text" id="article_physical_name"  name="article_physical_name" maxlength=80 physical_extension="<?php echo @$get_article_details['articleimageid']['physical_extension'];   ?>"  value="<?php echo @$get_article_details['articleimageid']['physical_name'];   ?> <?php if(@$utm_imgname!=''){ echo $utm_artimg[0]; } ?>" class="margin-top-5 WidthFull"  <?php if(isset($get_article_details['articleimageid']['temp_id'])) { ?> readonly <?php } ?>>
          </div>
		  <div class="ArticleImageContainer1 margin-right-0" id="relarticle_image_container" style=" margin-left: 25px;<?php if($page_name == 'edit_article' && $relatedimagedata['related_imagepath']==''){ echo 'display:none;'; } ?>">
		   <?php if($page_name == 'edit_article' && $relatedimagedata['related_imagepathid']!='' &&  $relatedimagedata['related_imagepathid']!=0){?>
			<script>$('#ArticleImageContainerId').show();$('#relhome_image_set').html('Change Image');$('#reledit_article_image').show();$('#reldelete_article_image').show();</script>
		   <?php } ?>
			<p style="">Preview</p>
			<img  id="relarticle_image_src" src="<?php if($page_name == 'edit_article' && $relatedimagedata['related_imagepath']!=''){ echo image_url.imagelibrary_image_path.$relatedimagedata['related_imagepath']; } ?>"  />
			<label>Caption :</label>
			<input type="text"  name="relarticle_image_caption" value="<?php if($page_name == 'edit_article' && $relatedimagedata['related_imagecaption']!=''){ echo $relatedimagedata['related_imagecaption']; } ?>"    class="margin-top-5 WidthFull" id="relarticle_image_caption" />
		   <label>Image Alt :</label>
		   <input type="text"  name="relarticle_image_alt" value="<?php if($page_name == 'edit_article' && $relatedimagedata['related_imagealt']!=''){ echo $relatedimagedata['related_imagealt']; } ?>"   class="margin-top-5 WidthFull"  id="relarticle_image_alt" />
		    <label style="width:50%;float:left;"><input style="float:left;width:auto;height:auto;" type="radio" name="related_image_type" value="0" <?php if($page_name == 'edit_article' && $relatedimagedata['related_image_type']!=''&& $relatedimagedata['related_image_type']=='0'){ echo 'checked'; } ?>>&nbsp; Image</label>
		   <label style="width:50%;float:left;"><input style="float:left;width:auto;height:auto;" type="radio" name="related_image_type" value="1"  <?php if($page_name == 'edit_article' && $relatedimagedata['related_image_type']!=''&& $relatedimagedata['related_image_type']=='1'){ echo 'checked'; } ?>>&nbsp; GFX</label>
		   <input type="hidden" id="temp_rel_image" name="temp_rel_image" value="<?php if($page_name == 'edit_article' && $relatedimagedata['related_imagepathid']!=''  &&  $relatedimagedata['related_imagepathid']!=0){ echo $relatedimagedata['related_imagepathid']; } ?>">
			</div>
        </div>
            ...
            <div class="FloatLeft">
            <input id="count1" maxlength="100" name="txtMetaTitle" type="text" value="<?php if(isset($get_article_details['MetaTitle'])) echo htmlentities($get_article_details['MetaTitle']); echo htmlentities(set_value('txtMetaTitle')); ?> <?php  if(isset($_POST['content_title'])) echo htmlentities($_POST['content_title']); ?>" class="box-shad1"/>
...
-->
 ...
 <input type="text" name="txtByLine" id="txtByLine"  value="<?php if(isset($select_author) && $select_author != "") {  echo htmlentities($select_author); } echo set_value('txtbyline'); ?> <?php  if(isset($_POST['content_authorname'])) echo htmlentities($_POST['content_authorname']); ?>" maxlength="50" />
    ...
    <?php echo form_error('ddCity'); ?>
           ...
           <input type="checkbox"  name="cbAllowComments" id="cbAllowComments"  <?php if(set_value("cbAllowComments") == 'on' || $page_name != 'edit_article' ||  (isset($get_article_details['Allowcomments']) && $get_article_details['Allowcomments'] == 1 ) ) echo  "checked"; ?> />
            <label for="cbAllowComments" class="margin-bottom-0">Allow Comments</label>
			  ...
			  <div class="padding-bottom-5">
           <input type="checkbox"  name="cbAllowPagination" id="cbAllowPagination"  <?php if(set_value("cbAllowPagination") == 'on' || $page_name != 'edit_article' ||  (isset($get_article_details['allow_pagination']) && $get_article_details['allow_pagination'] == 1 ) )  ?> />
...
<div class="remodal" data-remodal-id="modal5" data-remodal-options="hashTracking: false"  style="position:relative;">
	<div class="container">
		<ul class="nav nav-tabs">
			<li class="active"><a data-toggle="tab" href="#fromloc">From Local System</a></li>
			<li onclick="getrelimages(1);"><a data-toggle="tab" href="#fromlib">From Library</a></li>
		</ul>
		<div class="tab-content">
			<div id="fromloc" class="tab-pane fade in active">
				<div class="form-group" style="width:35%;float:left;">
					<form enctype="multipart/form-data" action="" method="post" id="fromlocform">
						<label style="font-weight: 700 !important;float: left;margin-top: 2%;">Select Image From Local system : (supports JPG only)</label>
						<input type="file" id="related_image" name="related_image" accept="image/*" class="form-control" style="border: 1px solid #c7bdbd !important;">
				</div>
				<div class="form-group" style="width:100%;float:left;text-align:left;">
					<input type="submit" value="submit" class="btn btn-primary" style="width:auto;color:#fff !important;">
				</div>
				</form>
			</div>
			<div  id="fromlib"  class="tab-pane fade" style="padding-top:2%;">
				<div class="col-md-12" style="margin-bottom:2%;">
					<input type="text" placeholder="Search" id="search_relatedimage" name="search_relatedimage" class="box-shad1 FloatLeft BrowseInput">
					<i id="reletedimage_search_id" class="fa fa-search FloatLeft BrowseSearch"></i>
				</div>
				<div class="left-related-image" style="width:75%;float:left;height: 355px !important;overflow-y:scroll;">
				</div>
				<div class="right-related-image" style="width:25%;float:left;height: 355px !important;">
					<div class="article_browse2 rel-hide" style="width:100%;height:300px;">
						<h4>Image Details</h4>
						<img id="relimage_path" src="">
						<h4 id="relimage_name"></h4>
						<p>Date:<span id="relimage_date"></span></p>
						<input type="hidden" value="" data-content_id="" data-image_alt="" data-image_caption="" data-image_date="" data-image_source="" data-image_path="" id="relbrowse_image_id" name="relbrowse_image_id">
						<div class="article_browse2_input">
							<label>Image Alt</label>
							<h5 id="reltextarea_alt"></h5>
							<label>Caption</label>
							<h5 id="reltextarea_caption"></h5>
						</div>
					</div>
					<div class="FloatRight popup_insert insert-fixed">
						<button type="button" class="btn btn-primary" id="relbrowse_image_insert">Insert</button>
					</div>
				</div>
			</div>
		 </div>
...
<script>
				var heightrel = 265;
				function getrelimages(type){
					$('.rel-hide').hide();
					if(type==1){ $count =0 ;}else{ $count = $('#relimagevalue').val();}
					var $search = $('#search_relatedimage').val();
					
					$.ajax({
						type:'post',
						cache:false,
						url:'<?php echo HOMEURL.folder_name ?>/article/getimage',
						data:{'count':$count,'search':$search},
						dataType:'json',
						success:function(result){
							if(type==1){  $('.left-related-image').html(result.template); heightrel = 265; }else{
								$('.left-related-image').append(result.template);
								$('#relimagevalue').val(result.count);
							}
						},
				...
				$('.left-related-image').on('scroll',function(){
					var containerheight = document.getElementsByClassName("left-related-image")[0].scrollHeight;
					console.log(containerheight);
				
					var containertop = $(this).scrollTop();
						console.log(containertop);
					var takecount = containerheight - containertop;
					console.log(heightrel);
					console.log(containertop);
					if(containertop >= heightrel){
							heightrel  =heightrel +600;
						getrelimages(2);
					}
				});
				$('#reletedimage_search_id').on('click',function(){
					getrelimages(1);
				});
				$(document).on('click','.relatedimg-sel',function(){
					var srcvalue = $(this).attr('src');
					var altval = $(this).attr('attr-alt');
					var captionval = $(this).attr('attr-caption');
					$('#relimage_path').attr('src',srcvalue);
					$('#temp_rel_image').val($(this).attr('attr-id'));
					$('#relimage_date').html($(this).attr('attr-date'));
					$('#reltextarea_alt').html(altval);
					$('#reltextarea_caption').html(captionval);
					$('.rel-hide').show();
				});
				$('#relbrowse_image_insert').on('click',function(){
					if($('#temp_rel_image').val()!=''){
						$('#relarticle_image_src').attr('src',$('#relimage_path').attr('src'));
						$('#relarticle_image_caption').val($('#reltextarea_caption').html());
						$('#relarticle_image_alt').val($('#reltextarea_alt').html());
						$('#relarticle_image_container').show().css('visibility','visible');
						$('#ArticleImageContainerId').show();
						$('#relhome_image_set').html('Change Image');
						$('#reledit_article_image').show();
						$('#reldelete_article_image').show();
						$('.remodal-close').trigger('click');
					}
				});
				$('#fromlocform').on('submit',function(e){
					e.preventDefault();
					var imagename = $('#related_image').val();
					if(imagename!='' && imagename!=undefined){
						$imglastindex=imagename.lastIndexOf(".");
						$CheckName=imagename.substr($imglastindex + 1).toLowerCase();
						if($CheckName=='jpg' || $CheckName=='jpeg'){
							var newform = new FormData();
							newform.append('related_image', $('#related_image')[0].files[0]);
							$.ajax({
								type:'post',
								url:'<?php echo HOMEURL.folder_name ?>/article/upload_relatedimage',
								data:newform,
								contentType:false,
								processData:false,
								cache:false,
								...
								success:function(result){
									...
									if(result.status=='sucess'){
										$('#temp_rel_image').val(result.contentid);
										$('#relarticle_image_src').attr('src','<?php echo image_url.imagelibrary_image_path ?>'+result.fullpath);
										$('#relarticle_image_caption').val('');
										$('#relarticle_image_alt').val('');
										$('#relarticle_image_container').show();
										$('#ArticleImageContainerId').show();
										$('#relhome_image_set').html('Change Image');
										$('#reledit_article_image').show();
										$('#reldelete_article_image').show();
										$('.remodal-close').trigger('click');
										toastr.error('Image uploaded successfully');
							...
							toastr.error('Please Upload JPG image file');
						}
					}else{
						toastr.error('Please Select a valid Image');
					}
				});
				$('#reldelete_article_image').on('click',function(){
					$('#relarticle_image_container').css('visibility','hidden');
					$('#temp_rel_image').val('');
					$('#relarticle_image_caption').val('');
					$('#relarticle_image_alt').val('');
					$('#relarticle_image_src').attr('src','');
					$('#reledit_article_image').hide();
					$('#reldelete_article_image').hide();
					$('#relhome_image_set').html('Set Image');
					toastr.success('Related image deleted successfully');
...
<script type="text/javascript" src="<?php echo image_url ?>js/article.js?top=58"></script>
...
// IE & DM Differents		
		...
		if( main_section_text  == 'Columns' ||  main_section_text  == 'Voices' ||  main_section_text  == 'ಅಂಕಣಗಳು')
			var author_type = 2;
		else 
			var author_type = 1; 
		...
		if(author_type != '' || parseInt(author_type) == 0) {
					...
					jqs("#txtByLine").autocomplete({
						...
						select: function( event, ui ) {
							...
							var lbl = ui.item.label.replace(' (byline)','');
							lbl = lbl.replace(' (columnist)','');
							jqs( "#txtByLine" ).val( lbl );
			...
			$.each(section_json,function(index,value){
				...
				if(content_type == 3) {
					  if(folder_name == 'niecpan')	
						...
						if(folder_name == 'niecpan')	
					  var condition = value.Sectionname == 'Videos' ;	  
						else 
					...
					if(folder_name == 'niecpan')	
					  var condition =  value.Sectionname != 'Galleries' && value.Sectionname != 'Videos' && value.Sectionname != 'Audios'
				  else 
...
$(document).ready(function(){
	var utmagen = '<?php echo @$_POST['utm_agency'] ?>';
	if(utmagen==''){utmagen='PTI';}
	$('#agency_id').find('option').each(function(){
		if($(this).html()==utmagen){
