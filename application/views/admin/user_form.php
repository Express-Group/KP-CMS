					...
					<div class="tabcontents">
						<div id="view3" style="display: block;">
							...
							<fieldset class="create-use-set">
								...
								<div class="qnsans">
									<div class="qns2" style="text-align:right;">
										<?php
											$agenciesaccess = '';
											if(isset($op_username) && isset($op_password) && $op_username!=''){
												$CI =&get_instance();
												$this->agencies_db = $CI->load->database('agencies_db',TRUE);
												$GetUser = $this->agencies_db->query("SELECT status FROM usermaster WHERE Username='".$op_username."' AND Password='".$op_password."'");
												if($GetUser->num_rows() > 0){
													$sts = $GetUser->result();
													if($sts[0]->status==1){
														$agenciesaccess = 'checked';
													}
												}
											}
										?>
										<input style="float: right;margin-top: 2px !important;" type="checkbox" name="agencies_right" <?php echo $agenciesaccess; ?>>
									</div>
									<div class="ans-use1">
										<label>Add user to agencies</label>
									</div>
								</div>
							</fieldset>
						</div>
