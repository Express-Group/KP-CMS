...
<textarea class="ckeditor"  id="article_head_line_id" name="txtArticleHeadLine"><?php echo set_value('txtArticleHeadLine'); ?><?php  if(isset($get_article_details['title'])) echo $get_article_details['title']; ?></textarea>
            ...
            <input type="text" class="box-shad1 full-width" id="txtUrlTitle" name="txtUrlTitle"  	  value="<?php if(isset($get_article_details['url_title'])) { echo htmlentities($get_article_details['url_title']); } echo set_value('txtUrlTitle'); ?>" />
             <?php echo form_error('txtUrlTitle'); ?>
			 <button type="button" class="btn btn-primary translate-url" style="float: right;margin: 7px 0 0 !important; padding: 3px 6px 2px !important;"><i class="fa fa-globe" aria-hidden="true"></i> translate</button>
 ...
 <?php if(isset($section_mapping)) { 
				...
				$section_bool = true;
				 ...
				 $condition = $mapping['Sectionname'] != 'ಫೋಟೊ ಗ್ಯಾಲರಿ' && $mapping['Sectionname'] != 'ವಿಡಿಯೋ' && $mapping['Sectionname'] != 'Audios' &&  $mapping['Sectionname'] != 'Resources';
...
<option id="MainSectionOption" style="color:#933;font-size:18px;"<?php /* if($mapping['Section_landing'] == 1 && $mapping['Sectionname'] != 'Columns' && $mapping['Sectionname'] != 'Magazine' && $mapping['Sectionname'] != 'The Sunday Standard' && $mapping['Sectionname'] != 'Editorials' ) { ?> disabled='disabled' <?php } */ ?>  class="blog_option" <?php if(set_value("ddMainSection") == $mapping['Section_id'] || (isset($get_article_details['content_id']) && $get_article_details['Section_id'] == $mapping['Section_id']  ) || ($page_name != 'edit_article' && $this->session->userdata('main_section') == $mapping['Section_id'] && folder_name == 'dmcpan')) { echo  "selected"; $section_bool=false; } ?> sectoin_data="<?php echo @$mapping['Sectionname']; ?>" rel="<?php echo @$mapping['LinkedToColumnist']; ?>"  value="<?php echo $mapping['Section_id']; ?>" url_structure="<?php echo mb_ucwords(str_replace("-"," ",str_replace("/"," > ",trim(@$mapping['BreadCrumb'])))); ?>" author_name="<?php echo (isset($mapping['AuthorID']) && $mapping['AuthorID'] != '')? get_authorname_by_id($mapping['AuthorID']) : '' ; ?>"><?php echo strip_tags($mapping['Sectionname']); ?></option>
 ...
 <?php echo form_error('ddMainSection'); ?>
  ...
  <input type="checkbox" name="cbSectionPromotion" id="SectionPromotion" class="HeightAuto"  <?php if((set_value("cbSectionPromotion") == 'on' && $page_name != 'edit_article') ||   (isset($get_article_details['section_promotion']) && $get_article_details['section_promotion'] == 1 ) ) echo  "checked"; ?>   <?php if($page_name != 'edit_article'){ echo 'checked'; } ?>> Section promotion
  ...
  <p id="mainsection_breadcrumb"> </p>
			...
			<label>Summary</label>
			<div id="cke_charcount_summary" length="300" class="charNum FloatRight" style="float:right;margin-bottom: 0;"><?php if(isset($get_article_details['summaryHTML'])) echo 300 - mb_strlen($get_article_details['summaryHTML']); else echo "300"?></div>
  ...
  <select name="ddAgency"  class="controls" id="agency_id"  >
   <option value="">-Select-</option>
	...
	$boolagency = true;
                      ...
                      <option value="<?php echo $agency->Agency_id; ?>" <?php if(set_value("ddAgency") == $agency->Agency_id || (isset($get_article_details['Agency_ID']) && $get_article_details['Agency_ID'] == $agency->Agency_id)) { echo  "selected='selected'"; $boolagency = false; } if($page_name != 'edit_article' && ( $agency->Agency_name == 'Online Desk')) { echo "selected='selected'"; $boolagency = false;  }?> ><?php echo ucfirst($agency->Agency_name); ?></option>
                      <?php } } 
    ...
    <?php echo form_error('ddCity'); ?>
           ...
           <input type="checkbox"  name="cbAllowComments" id="cbAllowComments"  <?php if(set_value("cbAllowComments") == 'on' || $page_name != 'edit_article' ||  (isset($get_article_details['Allowcomments']) && $get_article_details['Allowcomments'] == 1 ) ) echo  "checked"; ?> />
            <label for="cbAllowComments" class="margin-bottom-0">Allow Comments</label>
			  ...
			  <div class="padding-bottom-5">
           <input type="checkbox"  name="cbAllowPagination" id="cbAllowPagination"  <?php if(set_value("cbAllowPagination") == 'on' ||  (isset($get_article_details['allow_pagination']) && $get_article_details['allow_pagination'] == 1 ) ) echo  "checked"; ?> />
...
<div class="article_attribute">
	...
	<div class="article_tag"> 
		...
		<div class="example example_markup">
			<div id="seo_tagform_full"  class="bs-example1">
				<?php if(isset($seo_tags)) { 
					for($k=0;$k<count($seo_tags);$k++){ ?>
					<input type="text" name="seo_txtTags[]"   value="<?php echo $seo_tags[$k]; ?>" class="tag"/>
				<?php }  } else { ?>
					<input type="text" name="seo_txtTags[]"  value="" class="tag"/>
				<?php } ?>	
			</div>
		</div>
		</section> 
...
<script type="text/javascript" src="<?php  echo image_url ?>includes/Tagedit-master/js/jquery.tagedit.js"></script>
	...
	jqs(function() {
		...
		jqs( "#seo_tagform_full" ).find('input.tag').tagedit({
			autocompleteURL: "<?php echo base_url().folder_name; ?>/common/tags",
			allowEdit: false,
 			allowDelete: false
		});
...
$(document).ready(function(e){
	$('.translate-url').on('click' , function(e){
		toastr.remove();
		var urlTitle = $('#txtUrlTitle').val().trim();
		if(urlTitle!=''){
			var surl = "https://translation.googleapis.com/language/translate/v2?key=AIzaSyBmm4E74NnsoNTqRahoTfqbNvmB1mAh89A";
			surl += "&source=KN";
			surl += "&target=EN";
			surl += "&q=" +urlTitle;
			$.get(surl, function (data, status) {
				//console.log(surl);
				$('#txtUrlTitle').val(data.data.translations[0].translatedText);
				toastr.success('Url Title Translated Successfully');
			});
		}else{
			toastr.error('Enter Valid Url Title');
		}
	});
});
</script>