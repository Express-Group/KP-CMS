...
<!--<script type="text/javascript" src="<?php echo $script_url; ?>js/jquery-1.11.3.min.js"></script>	
...
<script type="text/javascript" src="<?php echo $script_url; ?>js/astro_daily_predictions.js"></script>

<div class="Container">
<div class="BodyWhiteBG">
<div class="BodyHeadBg Overflow clear">
<div class="FloatLeft  BreadCrumbsWrapper PollResult">
<div class="breadcrumbs"><a href="<?php echo base_url(folder_name); ?>">Dashboard</a> > <a href="#"><?php echo $title; ?></a></div>
...
<?php $data['Menu_id'] = get_menu_details_by_menu_name('Daily Predictions'); ?>
<?php if(defined("USERACCESS_ADD".$data['Menu_id']) && constant("USERACCESS_ADD".$data['Menu_id']) == '1') { ?>
 <p class="FloatRight SaveBackTop"><a href="<?php echo base_url(folder_name);?>/astro_daily_prediction/add_daily_predictions" class="btn-primary btn"><i class="fa fa-file-text-o"></i> &nbsp;Add New</a></p>
<?php } ?>
</div>
...
function get_astro_predictions() {
	...
	$('#example').dataTable({
		...
		"searching": false,
		"iDisplayLength": 10,
		"aaSorting": [[0,'desc']], // Default sorting for table
		"fnDrawCallback":function(oSettings){
		   $("html, body").animate({ scrollTop: 0 }, "slow");
			...
			if($(this).find('tbody tr').text()== "No matching records found")
			{
				...
				$("#example").find('tbody tr').append($('<td class="BackArrow"><a href="<?php echo base_url(folder_name); ?>/astro_daily_prediction" data-toggle="tooltip" title="Back to list"><i class="fa fa-reply fa-2x"></i></a></td>'));
			}
			else
			{
		...
		"ajax": {
			"url": "<?php echo base_url(folder_name); ?>/astro_daily_prediction/get_astro_dailypredictions",
			"type" : "POST",
			"data" : {
			"rasi_id" : rasi_id, "from_date" : check_in, "to_date" : check_out}
		}
	});
	...
	setTimeout(function(){$('#flash_msg_id').hide();}, 2000);
</script>