					...
					<ul class="AskPrabuCheckBox FloatLeft">
						...
						<li>
							<p class="CalendarWrapper" id="checkin_checkout_div">
								...
								<button id="pr_report" class="btn btn-primary">Productivity Report</button>
							</p>
						</li>
...
$(document).ready(function()
{
	...
	$('#pr_report').on('click' , function(e){
		toastr.remove();
		var fdate=$( "#date_timepicker_start").val();
		var tdate=$( "#date_timepicker_end").val();
		if(fdate!='' && tdate!=''){
			$.ajax({
				type : 'post',
				url : '<?php echo base_url(folder_name."/user_productivity_report/article_prod_report") ?>',
				data: {'fromdate' : fdate , 'enddate' : tdate},
				cache:false,
				success:function(result){
					$('#pr_modal').find('.modal-body').html(result);
					 $("#pr_modal").modal({backdrop: "static"});
				},
				error:function(err , errcode){
					toastr.error('status Code : ' +err.status+ ', Status : ' + err.statusText);
				}
			})
		}else{
			toastr.error('Select start date & end date');
		}
	});
});
</script> 
