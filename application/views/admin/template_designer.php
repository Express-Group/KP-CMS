...
<div class="BodyHeadBg Overflow clear">
        ...
        <?php } ?>
			...
			<span class="FloatRight save_margin article_save ">                
                <button style="height: 39px;" id="common_headers" name="common_headers" title="Common Headers" class="btn btn-primary FloatLeft right-wid save-margin-right common_header ">Headers</button>
            </span>
											...
											echo '<li class="'.$have_child_sections.'" '. @$sub_attribs[1] .'>'. $submenu_value['Sectionname'];											
												...
												if(isset($special_menu_details))
												{
													foreach($special_menu_details as $special_menu)
													{
														...
														echo "<ul>";
														echo $landing;
														$GrandChildren = $this->db->query("SELECT Section_id, Sectionname FROM sectionmaster WHERE ParentSectionID='".$special_menu_attribs_array['menuid']."' AND IsSubSection='1' AND Status='1'")->result();
														if(count($GrandChildren) > 0){
															foreach($GrandChildren as $childs):
																$GrandChildrenDetails = $this->db->query('CALL get_pagemaster_sections("'.$childs->Section_id.'")')->result_array();
																$attr = 'id="'.@$GrandChildrenDetails[0]['id'].'" data-sectionId="'.@$GrandChildrenDetails[0]['menuid'].'" data-pageType="'.@$GrandChildrenDetails[0]['pagetype'].'" data-pageId="'.@$GrandChildrenDetails[0]['id'].'" data-hasTemplate ="'.@$GrandChildrenDetails[0]['hasTemplate'].'" ';
																echo '<li class="" '. @$attr.'>'.$childs->Sectionname.'</li>';
															endforeach;
...
<script type="text/javascript" src="<?php echo $frontend_js; ?>bootstrap.min.js"></script>
...
#common_head  textarea{
	height: 300px;
    border: 1px solid #ccc !important;
    max-height: 300px;
}
#common_head #script_save{
	background-color: #337ab7;
    border-color: #2e6da4;
    color: #fff;
}
</style>
<div class="modal fade" id="common_head" role="dialog">
    <div class="modal-dialog" style="top:10%;">
      <div class="modal-content">
        <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal">&times;</button>
          <h4 class="modal-title"><b>COMMON HEADERS</b></h4>
        </div>
		<form method="post" id="script_form" action="">
        <div class="modal-body">
			<div class="permission-denied">
				<h4 class="text-center" style="font-weight: 700 !important;color: red;">YOU DON'T HAVE ACCESS TO VIEW</h4>
			</div>
			<ul class="nav nav-tabs">
				<li class="active"><a data-toggle="tab" href="#homeheader">Home Page</a></li>
				<li><a data-toggle="tab" href="#sectionheader">Section Page</a></li>
				<li><a data-toggle="tab" href="#articleheader">Article Page</a></li>
		    </ul>
			 <div class="tab-content">
				<div id="homeheader" class="tab-pane fade in active">
				  <div style="margin-top:2%;">
					<label>Script / content</label>
					<textarea name="home_script" class="form-control"></textarea>
				  </div>
				</div>
				<div id="sectionheader" class="tab-pane fade">
				  <div style="margin-top:2%;">
					<label>Script / content</label>
					<textarea name="section_script" class="form-control"></textarea>
				  </div>
				</div>
				<div id="articleheader" class="tab-pane fade">
				  <div style="margin-top:2%;">
					<label>Script / content</label>
					<textarea name="article_script" class="form-control"></textarea>
				  </div>
				</div>
			</div>
        </div>
        <div class="modal-footer">
          <button id="script_save" type="button" class="btn btn-default">Save</button>
...
$(document).ready(function(e){
	$('#common_headers').on('click' , function(e){
		$.ajax({
			type:'post',
			cache:false,
			data:{},
			url:'<?php echo base_url(folder_name) ?>/template_designer/get_common_script',
			dataType:'json',
			success:function(result){
				if(result.has_access==1){
					$('#common_head').find('.nav-tabs').show();
					$('#common_head').find('.tab-content').show();
					$('#common_head').find('#script_save').show();
					$('#common_head').find('.permission-denied').hide();
					$('textarea[name="home_script"]').val(result.home_header_script);
					$('textarea[name="section_script"]').val(result.section_header_script);
					$('textarea[name="article_script"]').val(result.article_header_script);
				}else{
					$('#common_head').find('.nav-tabs').hide();
					$('#common_head').find('.tab-content').hide();
					$('#common_head').find('#script_save').hide();
					$('#common_head').find('.permission-denied').show();
				}
			},
	...
	$('#script_save').on('click' , function(e){
		...
		if(cnf){
			$.ajax({
				...
				url:'<?php echo base_url(folder_name) ?>/template_designer/common_scripts',
				success:function(result){
					if(result==1){
						toastr.success('Header scripts updated successfully');
						$("#common_head").modal("toggle");
					}else{
						toastr.error('Something went wrong.please try agian');
					}
				},
