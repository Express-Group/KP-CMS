...
<div class="FloatLeft w2ui-field">
 ...
 <?php if(isset($section_mapping)) { 
				 ...
				 if(folder_name == "niecpan")
					$condition = $mapping['Sectionname'] == 'ಫೋಟೊ ಗ್ಯಾಲರಿ' ;
				 else
		...
		if(parseInt(Object.keys(publish_object).length) != 0) {
			...
			if(publish_status == true) {
				...
				$.ajax({ 
					...
					type:"POST",
					data:{"contentids" : content_data,"content_type" : 3},
...
$("#flash_msg_id").slideDown(function() {
    ...
    $('#example').dataTable( {
		...
		"ajax": {
            "url": "<?php echo base_url().folder_name; ?>/gallery_manager/gallery_datatable",
			"type" : "POST",
			"data" : {
		...
		//	$("#checkin_checkout_div").hide();
			...
			var alert_name = 'gallery';
			 ...
			 $(document.body).on('click', '#status_change', function(event)
			 {
				...
				type:"POST",
				data:{"contentid":id, "status":final_status,"content_type":3},
				success: function(data) 
					{
						if(data == 'success')
						{
