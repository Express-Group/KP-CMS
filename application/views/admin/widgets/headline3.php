<style>.breaking-type-3{margin:7px 0 7px;}.breaking-type-3 p ,.breaking-type-3 p a{border-bottom:none;color: #626262!important;font-size: 13px;}</style>
<?php 
	...
	if(count($widget_contents) > 0 ){
		...
		foreach($widget_contents as $get_content)
		{
		   ...
		   $custom_summary  = preg_replace('/<p[^>]*>(.*)<\/p[^>]*>/i', '$1',$custom_summary);  //to remove first<p> and last</p>  tag
			...
			/*if($i == 1)
			{*/
				$show_simple_tab .='<figure><a  href="'.$live_article_url.'"  class="article_click" >'.$img_icon.'<img src="'.$dummy_image.'" data-src="'.$show_image.'" width="600" height="300"></a></figure>';
