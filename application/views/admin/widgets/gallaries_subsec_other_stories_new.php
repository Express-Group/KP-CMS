...
if (strpos(strtolower($section_details['URLSectionStructure']), 'photogallery') !== false) 
{			
$content_type = 3;
}
else
...
$subsection_widgetID = $this->widget_model->get_widget_byname('Gallery / Video subsection main widget', $view_mode);
...
if($content_type == 3)
$show_simple_tab.=	'<h4 class="topic">Other Galleries</h4>';
else
$show_simple_tab.=	'<h4 class="topic">Other Videos</h4>';	
...
if($subsec_leadstory_remdering_mode == "manual")
{
	...
	if(count($get_widget_instance_contents)>0)
	{
		...
		$widget_instance_contents = $this->widget_model->get_liveContents_by_sectionId($article_count, $page_section_id, $view_mode, $article_limit, $subsec_leadstory_remdering_mode, $multiple_contentID, $content_type, $is_home);	
		$total_data = $this->widget_model->get_liveContents_by_sectionId($max_article_count, $page_section_id, $view_mode, $subsec_leadstory_max_article, $subsec_leadstory_remdering_mode, $multiple_contentID, $content_type, $is_home);
	...
	else
	{
		$leadstory_contentID = '';
		$article_limit = ($this->input->get('per_page') != '') ? $this->input->get('per_page'):0;
		$widget_instance_contents = $this->widget_model->get_liveContents_by_sectionId($article_count, $page_section_id, $view_mode, $article_limit, $subsec_leadstory_remdering_mode, $leadstory_contentID, $content_type, $is_home);		
		$total_data = $this->widget_model->get_liveContents_by_sectionId($max_article_count, $page_section_id, $view_mode, $subsec_leadstory_max_article, $subsec_leadstory_remdering_mode, $multiple_contentID, $content_type, $is_home);
		$TotalCount = count($total_data);
	}
}
else
{
	...
	{
		$get_widget_instance_contents = $this->widget_model->get_all_available_articles_auto($subsec_leadstory_max_article, $page_section_id , $content_type ,  $content['mode'], $is_home);
		if(count($get_widget_instance_contents)>0)
		{
			$leadstory_contentID = array();	
			foreach($get_widget_instance_contents as $leadstory_contentid)
...
if(count($widget_contents) > 0)
{
	...
	foreach($widget_contents as $get_content)
	{
		...
		$gallery_icon= image_url. imagelibrary_image_path.'gallery-icon.png';
			...
			$show_simple_tab.= '<div class="col-lg-4 col-md-4 col-sm-4 col-xs-12"><div class="GalleryDetailListnew">';
			...
			if($content_type == '3'){
					$show_simple_tab.='<img class="GalleryVideoSubIcon"  src="'.$gallery_icon.'">';
			}else if($content_type == '4'){
					$show_simple_tab.='<img class="GalleryVideoSubIcon"  src="'.$play_video_image.'">';
			}
