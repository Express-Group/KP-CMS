...
$show_simple_tab        .='<div class="lead_stories features lead_stories_vertical" '.$widget_bg_color.' id="lead_stori_'.$widget_instance_id.'">';
	...
	{
		$show_simple_tab.=	'<h4 class="subtopic subtopic-title"><a href="'.$widget_section_url.'">'.$widget_custom_title.'</a></h4>';
	}
	else
	{
		if(trim($widget_custom_title) != '' && trim(strtolower($widget_custom_title))!='none')
		$show_simple_tab.=	'<h4 '.((strtolower($widget_custom_title)=="live")? 'style="color:red;"' : '').' class="'.((strtolower($widget_custom_title)=="live")? 'live-xs ' : '').'subtopic subtopic-title subtop">'.$widget_custom_title.'</h4>';
		else 
		...
		else 
		{
			$show_image	= image_url. imagelibrary_image_path.'logo/nie_logo_600X390.jpg';
				...
				$show_simple_tab .='<div class="item '.$add_active.'">
									<a  href="'.$live_article_url.'"  class="article_click" >'.$img_icon.'<img  src="'.$show_image.'"  title = "'.$imagetitle.'" alt = "'.$imagealt.'"></a><h4 class="subtopic subtopic-vertical"><a  href="'.$live_article_url.'"  class="article_click" >'.$display_title.'</a></h4>';
				...
				if($content['RenderingMode'] == "manual"){
					...
					if(($static_id==$get_content['content_id'])){
						$title1= "Be your own man: Ratan Tata's advice to Cyrus Mistry";
						$title2 = "Ratan Tata's legacy, Cyrus Mistry's challenge";
						$show_simple_tab .= '<p>  <i class="fa fa-circle"></i> <a  href="http://www.newindianexpress.com/business/2012/dec/14/be-your-own-man-ratan-tatas-advice-to-cyrus-mistry-433834.html" class="article_click" >'.$title1.'</a>';
						$show_simple_tab .= '<p>  <i class="fa fa-circle"></i> <a  href="http://www.newindianexpress.com/business/2012/dec/28/ratan-tatas-legacy-cyrus-mistrys-challenge-437781.html" class="article_click" >'.$title2.'</a>';
					}
	
				}
				$show_simple_tab .='</div><hr></div>'; 
