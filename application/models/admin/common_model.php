...
{	
	...
	public function get_tags_by_order($tags) {
		$class_object =  new tags_model;
		return $class_object->get_tags_by_order($tags);
	}
...
class tags_model extends Common_model 
{
	...
	public function get_tags_by_order($tags) {
		...
		for($i=0;$i<count($splitTags);$i++):
			$TagId = str_replace('IE','',$splitTags[$i]);
			$result =$this->db->query("SELECT tag_id , tag_name FROM tag_master WHERE tag_id='".$TagId."'")->result();
			$temp[$i] = new stdClass;
			$temp[$i]->tag_id = $result[0]->tag_id;
			$temp[$i]->tag_name = $result[0]->tag_name;
			array_push($generatedTags , $temp[$i]);
			unset($temp[$i]);
		endfor;
...

{
	public function common_resize_all_images($ImageDetails) {
		...
		{	
			...
			foreach($ImageDetails as $key => $ArrayImage) {
				...
				//added for new dimension 900X450
				$Image900X450 	= str_replace(".","_900_450.", $image_name);
 ...
 $src_h);
						*/	
						...
						//added for new dimension
						if (!file_exists(source_base_path . $TempSourceURL . $Image900X450) || $save_status == 0) {
							$dst_w 		= 900;
							$dst_h 		= 450;
							$dst 		= $ImagePath. str_replace(".","_900_450.",$image_name);	
						    $source_ratio = $src_w / $src_h; 
							$destination_ratio = $dst_w / $dst_h; 
							// crop to fit 
							if ($source_ratio > $destination_ratio) { 
