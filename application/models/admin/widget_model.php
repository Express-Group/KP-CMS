...
class Widget_model extends CI_Model
{
  ...
  public function __construct() 
	...
	public function GetMetaData($value){
		$Query=$this->db->query("SELECT meta_name,meta_description FROM tag_master WHERE tag_name='".$value."'");
		if($Query->num_rows()==0){
			$data['name']='';
			$data['description']='';
		}else{
			$res=$Query->result();
			$data['name']=$res[0]->meta_name;
	...
	public function AmpStatus($ContentID=''){
	 ...
	 public function get_tab_sectiondetails($instance_id,$view_mode)

	{
		$class_object = new Widget_config_class;
		return $class_object->get_tab_sectiondetails($instance_id,$view_mode);
	}
	 ...
	 public function get_sponsorarticles(){
		$class_object = new Widget_config_class;
		return $class_object->get_sponsorarticles();
	}
    ...
    public function get_gallery_images_by_id($content_id)
	 ...
	 public function get_Stories_For_Author_all($view_mode,$auth_name)
	 {
	  $class_object = new Widget_config_class;
	  return $class_object->get_Stories_For_Author_all($view_mode,$auth_name);
	 }
	...
	public function get_related_image_by_contentid($content_id, $view_mode='') {
	   $class_object = new Widget_content_class;
		return $class_object->get_related_image_by_contentid($content_id, $view_mode='');
	}
 ...
 public function get_content_type($content_id)
	...
	public function GetLiveNowContent($type,$article_id=0,$image_url=0){
		$class_object = new Widget_config_class;
		return $class_object->GetLiveNowContent($type,$article_id,$image_url);
	}
	
	public function Get_dynamic_tables(){
		$class_object = new Widget_config_class;
		return $class_object->Get_dynamic_tables();
	}
	public function GetLiveNewsCount($article_id,$section_id){
		$class_object = new Widget_config_class;
		return $class_object->GetLiveNewsCount($article_id,$section_id);
	}
	
	public function Get_Dynamic_Table_Data($tid=1){
		$class_object = new Widget_config_class;
		return $class_object->Get_Dynamic_Table_Data($tid);
	}
	
	public function GetAuthorArchiveDetails($author_name,$SectionID,$type){
		$class_object = new Widget_config_class;
		return $class_object->GetAuthorArchiveDetails($author_name,$SectionID,$type);
	}
	
	public function GetContentBasedOnArchive($limit,$author_name,$totalcount){
		$class_object = new Widget_config_class;
		return $class_object->GetContentBasedOnArchive($limit,$author_name,$totalcount);
	}
	public function GetSectionMappingId($section_id,$content_type_id,$content_id){
		$class_object = new Widget_config_class;
		return $class_object->GetSectionMappingId($section_id,$content_type_id,$content_id);
	}
}
...
class Widget_config_class extends Widget_model
{
	...
	{
		if($view_mode=='live'){
		$CacheID ="CALL get_widget_tabsection_details('".$instance_id."')";
		if(!$this->memcached_library->get($CacheID) && $this->memcached_library->get($CacheID) == '') {	
		$query 	= $this->live_db->query("CALL get_widget_tabsection_details('".$instance_id."')");
		...
		$select_query 	= $this->db->query("CALL get_widget_tabsection_details('".$instance_id."')")->result_array();		
		}
		return $select_query;
	}
	public function GetSectionMappingId($section_id,$content_type_id,$content_id){
		...
		if($content_type_id==1){
			$Query=$this->live_db->query("SELECT section_id FROM article_section_mapping WHERE content_id='".$content_id."'");
			if($Query->num_rows()==0){
				return 0;
			}else{
				foreach( $Query->result() as $data){
					array_push($result,$data->section_id);
				}
				return $result;
			}
		}else{
	...
	public function get_sponsorarticles(){
		return $this->live_db->query("SELECT a1.title,a1.url FROM article as a1 LEFT JOIN sponsor_article as a2 ON a1.content_id=a2.content_id WHERE a2.expiry_date >=NOW() AND a2.status=1 ORDER By a2.display_order ASC ")->result();
	}
	...
	public function GetAuthorArchiveDetails($author_name,$SectionID,$type){
		...
		$year_range = range($start_range, '2013');
		$result_count=0;
		foreach($year_range as $year)
		{
			...
			{
				if($type==1){
					$query = $archive_db->query("SELECT content_id FROM ".$tablename." WHERE author_name='".$author_name."'")->num_rows();
					if($query > 0){
						$result_count=$query;
						break;
					} 
				...
				if($type==2){
					$query = $archive_db->query("SELECT content_id FROM ".$tablename." WHERE author_name='".$author_name."'")->num_rows();
					 $result_count = ($result_count + $query);
				}
	...
	public function GetContentBasedOnArchive($limit,$author_name,$totalcount){
		...
		$year_range = range($start_range, '2013');
		$query='';
		$count=count($year_range);
		$i=1;
		foreach($year_range as $year){
			$tablename = $table.'_'.$year;
			if($archive_db->table_exists($tablename)){
				if($count==$i):
				$query .="SELECT title,url,summary_html,last_updated_on FROM ".$tablename." WHERE author_name='".$author_name."'";
				else:
				$query .="SELECT title,url,summary_html,last_updated_on FROM ".$tablename." WHERE author_name='".$author_name."'  UNION ";
				endif;
			}
		$i++;
		}
		$query .=" ORDER BY last_updated_on DESC LIMIT ".$limit.",".$totalcount;
		return $archive_db->query($query)->result();
	}

	public function Get_Dynamic_Table_Data($tid){
		$this->load->database();
		return  $this->db->query("SELECT tid,table_name,table_properties,total FROM tablemaster WHERE status=0 AND tid='".$tid."'")->result();
	}

	public function GetLiveNewsCount($article_id,$section_id){
		if($section_id==161){
			$FileName= $article_id.'.json';
	...
	public function GetLiveNowContent($type,$article_id,$image_url){
		$this->load->database();
		if($type==1){
			$status=1;
			$Result=$this->db->query("CALL get_live_now_content('".$status."','".$article_id."')")->result();
			$Template='';
			foreach($Result as $Data):
				if($Data->update_status==1):
				$status=2;
				$this->db->cache_on();
				$this->db->query("CALL get_live_now_content('".$status."','".$Data->lid."')")->result();
				$this->db->cache_off();
				endif;
				$Template .='<div class="live-inner-content lid_'.$Data->lid.'">';
				$Date=explode(' ',$Data->created_on);
				$Date=explode(':',$Date[1]);
				$Date=$Date[0].':'.$Date[1];
				$Time=strtotime($Data->created_on);
				$Time=Date('M j',$Time);
				$Template .='<span class="livenow-title">'.$Date.' '.$Time.' </span>';
				$Template .='<div class="livenow-socialicons"><a target="_blank" href="https://www.facebook.com/sharer/sharer.php?u='.BASEURL.'&title='.$Data->title.'&picture='.$image_url.'"><i class="fa fa-facebook" aria-hidden="true"></i></a><a target="_blank" href="https://twitter.com/intent/tweet?text='.$Data->title.' via @NewIndianXpress"><i class="fa fa-twitter" aria-hidden="true"></i></a></div>';
				$Template .='<h3 class="livenow_h3">'.$Data->title.'</h3>';
				$Template .='<div class="livenow-description">'.$Data->content.'</div>';
				$Template .='<input type="hidden" id="l_'.$Data->lid.'" value="'.$Data->lid.'">';
		...
		if($type==3){
			$status=3;
			$Content=$this->db->query("CALL get_live_now_content('".$status."','".$article_id."')")->result();
			 $template=array();
			$full_arr=array();
			 foreach($Content as $Data):
				$template['lid']=$Data->lid;
				$template['title']=$Data->title;
				$template['content']=$Data->content;
				$template['created_on']=$Data->created_on;
				$full_arr[]=$template;
			endforeach; 
			return json_encode($full_arr);
		}
	...
	public function Get_dynamic_tables(){
		$this->load->database();
		$Query=$this->db->query("SELECT tid,table_name,table_properties,total FROM tablemaster WHERE status='0' AND (table_properties!=NULL OR table_properties!='')");
		if($Query->num_rows() > 0){
			return $Query->result();
		}else{
			return 'NO ROWS';
 ...
 public function get_liveContents_by_sectionId($max_article, $section_id, $view_mode, $lead_story_maximum_Articles , $subsec_leadstory_remdering_mode, $leadstory_contentID, $contentType_id,$is_home)
{
 ...
 public function get_Stories_For_Author($max_article, $section_id, $view_mode,$authorid)
 {
  ...
  public function get_Stories_For_Author_all($view_mode,$authname)
 {
	$article_manager  =  $this->live_db->query("select  content_id,section_id from article  where status = 'P' AND publish_start_date <=NOW() AND  author_name='".$authname."'");
	$result = $article_manager->result_array();
     return $result;
 }
 ...
 public function get_RightSide_OtherStories_Contents_Type3($max_article, $section_id, $view_mode,$sectionid,$parentsectionid)
 {
	...
	if($content_type==7){
		$rss_article =$this->db->query("SELECT Question_id,Questiontext,AnswerInPlainText,UserName,Place,SubmittedOn FROM askprabhu WHERE Status='1' ORDER BY Question_id DESC LIMIT 100")->result_array();
	}else{
...
class Widget_content_class extends Widget_config_class {
	...
	public function get_related_image_by_contentid($content_id, $view_mode='') {
		...
		$relatedimages =  $this->live_db->query("SELECT related_imagepath, related_imagealt, related_imagecaption, related_image_type from  article WHERE content_id= '".$content_id."'");
		return ['count' =>$relatedimages->num_rows() , 'result' => $relatedimages->result() ];
	}
	public function update_most_hits_and_emailed($type, $content_type, $content_id, $title, $section_id, $content_created){
