...
class userdetails extends User_Model
{
	...
	*/
	...
	//	$altemailid=$this->input->post('txtAltEmailID');
		...
		extract($_POST);
		$CI = &get_instance();
		$this->agencies_db  = $CI->load->database('agencies_db',TRUE);
		$GetUser = $this->agencies_db->query("SELECT User_id FROM usermaster WHERE Username='".$username."'");
		$agenciesdata = array("role_id"=>$roleid , "domain_name"=>"ENS" ,"status"=>$status,"Createdby"=>$userid,"Createdon"=>$createdon,"Modifiedby"=>$userid,"Modifiedon"=>$createdon,"Username"=>$username ,"Firstname"=>$firstname , "Lastname"=>$lastname ,"Mobileno"=>$mobileno ,"MailId"=>$emailid,"Employeecode"=>$employeecode);
		if($encrypted_string!=''){
			$agenciesdata["Password"] = $encrypted_string;
		}
		if(isset($agencies_right) && $agencies_right=='on'){
			if($GetUser->num_rows() > 0){
				$Agencies = $GetUser->result();
				$AgenciesID = $Agencies[0]->User_id;
				$this->agencies_db->where('User_id' ,$AgenciesID);
				$this->agencies_db->update('usermaster',$agenciesdata);
			}else{
				$this->agencies_db->insert('usermaster',$agenciesdata);
			}
		}else{
			if($GetUser->num_rows() > 0){
				$Agencies = $GetUser->result();
				...
				$this->agencies_db->where('User_id' ,$AgenciesID);
				$this->agencies_db->update('usermaster',$adata);
			}
		}
		if($hdn_user_id=="")
		{
