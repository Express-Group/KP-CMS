...

{
	...
	*/
...

	{
		...
		if(isset($seo_txtTags) != ''){
			$seo_txtTags = implode(",",$seo_txtTags);
		}else{
			$seo_txtTags = '';
		}
		$null_value 		= "NULL";
			...
			if($article_id != '' && $article_id != 'NULL') {
				...
				if($seo_txtTags!=''){
					$this->live_db->insert('seo_tags', ['tag_name' => $seo_txtTags,'content_id' => $article_id,'updated_by' => USERID]);
				}
				if ($txtStatus == 'P') {
				...
				$relatedimagepath='';
				 if($temp_rel_image!='' && $temp_rel_image!=0){
					if($related_image_type==''){ $related_image_type=0;}
					$relatedimage = $this->db->query('SELECT ImagePhysicalPath FROM imagemaster WHERE content_id="'.$temp_rel_image.'"')->result();
					$relatedimagepath = @$relatedimage[0]->ImagePhysicalPath;
					$this->db->query("UPDATE articlemaster SET related_imagepath='".$relatedimagepath."', related_imagepathid='".$temp_rel_image."', related_imagealt='".addslashes($relarticle_image_alt)."', related_imagecaption='".addslashes($relarticle_image_caption)."', related_image_type='".$related_image_type."' WHERE content_id='".$article_id."'");
					$this->live_db->query("UPDATE article SET related_imagepath='".$relatedimagepath."', related_imagealt='".addslashes($relarticle_image_alt)."', related_imagecaption='".addslashes($relarticle_image_caption)."', related_image_type='".$related_image_type."' WHERE content_id='".$article_id."'");
					
				 }else{
					$this->db->query("UPDATE articlemaster SET related_imagepath='', related_imagepathid='', related_imagealt='', related_imagecaption='', related_image_type='0' WHERE content_id='".$article_id."'");
					$this->live_db->query("UPDATE article SET related_imagepath='', related_imagealt='', related_imagecaption='', related_image_type='0' WHERE content_id='".$article_id."'");
				 }
			...
			if ($this->db->trans_status() === FALSE || $this->live_db->trans_status() === FALSE) {
				...
				if(isset($todayspaper) && $todayspaper=='on'){
					$this->live_db->insert("todayspaper",["content_id"=>$article_id,"section_name"=>$article_details['LiveArticleDetails']["section_name"],"title"=>trim(addslashes($txtArticleHeadLine)),"url"=>addslashes($article_details['url']),"publish_start_date"=>trim($article_details['PublishStartDate']),"status"=>$txtStatus,"created_by"=>USERID,"section_id"=>$ddMainSection]);
				}
				$articlelog=[];
				$articlelog['details'] = $article_details;
				$articlelog['rawcontent'] = $_POST;
				$articlelogjson = json_encode($articlelog);
				$this->db->insert("article_log",["log"=>$articlelogjson,"created_by"=>USERID,"article_id"=>$article_id]);
				//to save article page as html
				 /* if($txtStatus == 'P'){
					$response = $this->getarticle_viacurl(BASEURL.$article_details["url"]);
					if($response!='' && $response!=null):
						$sourcepath = FCPATH."/application/views/view_template/article/".$article_id.".php";
						$originalsourcepath = $article_id.".php";
						file_put_contents($sourcepath ,$response);
						chmod($sourcepath,0777);
						$this->savearticlecontent($originalsourcepath ,$response);
					endif;
				}  */
				//end..(to save article page as html)
				if(@$utm_desk_id!=''){
					$CI = &get_instance();
					$this->printdesk_db = $CI->load->database('printdesk_db', TRUE);
					$printpubDate = date('Y-m-d H:i:s');
					$this->printdesk_db->where('desk_id',@$utm_desk_id);
					$this->printdesk_db->update('printdesk_articlemaster',['status'=>1,'article_status_msg'=>$txtStatus,'user_id'=>USERID,'content_id'=>$article_id,'user_first_last_name'=>$utm_firstname.'||'.$utm_lastname,'updated_on'=>$printpubDate]);
				}
				if($publish_close == 1) 
	...
	public function gettodayspaperstatus($article_id){
		return $this->live_db->query("SELECT tid FROM todayspaper WHERE content_id='".$article_id."'")->num_rows();
	}
	
	public function __getimage($count,$search){
		if($search!=''){
			return $this->db->query("SELECT content_id,Createdon,ImageCaption,ImageAlt,ImagePhysicalPath FROM imagemaster WHERE ImageCaption LIKE '%".$search."%' ORDER BY content_id DESC LIMIT ".$count.", 30 ")->result();
		}else{
			return $this->db->query("SELECT content_id,Createdon,ImageCaption,ImageAlt,ImagePhysicalPath FROM imagemaster ORDER BY content_id DESC LIMIT ".$count.", 30 ")->result();
		}
			
	}
	public function getrelatedimagedata($article_id){
		$response =  $this->db->query("SELECT related_imagepath,related_imagepathid,related_imagealt,related_imagecaption,related_image_type FROM articlemaster WHERE content_id='".$article_id."'");
		if($response->num_rows()==0){
			return ['related_imagepath'=>'','related_imagepathid'=>'','related_imagealt'=>'','related_imagecaption'=>'','related_image_type'=>''];
		}else{
			$result = $response->result();
			if($result[0]->related_imagepathid!='' && $result[0]->related_imagepathid!=0){
				return ['related_imagepath'=>$result[0]->related_imagepath,'related_imagepathid'=>$result[0]->related_imagepathid,'related_imagealt'=>$result[0]->related_imagealt,'related_imagecaption'=>$result[0]->related_imagecaption,'related_image_type'=>$result[0]->related_image_type];
			}else{
				return ['related_imagepath'=>'','related_imagepathid'=>'','related_imagealt'=>'','related_imagecaption'=>'','related_image_type'=>''];
			}
		}
	...
	//to send article page details via curl
	public function savearticlecontent($sourcepath ,$response){
		$post_data = array('file_name' => $sourcepath,'file_contents'=> $response);
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, BASEURL.'user/commonwidget/save_article_content');
	...
	public function getarticle_viacurl($url){
		$ch = curl_init();
	...
	//end..(to send article page details via curl)
...

		{
			...
			*/
			...
			$article_details['content_id'] = $article_id;
				...
				$seorows = $this->live_db->query("SELECT content_id FROM seo_tags WHERE content_id='".$article_id."'")->num_rows();
				if($seorows==0){
					$this->live_db->insert('seo_tags', ['tag_name' => $seo_txtTags, 'updated_by' => USERID , 'content_id'=> $article_id]);
				}else{
					$this->live_db->where('content_id', $article_id);
					$this->live_db->update('seo_tags', ['tag_name' => $seo_txtTags, 'updated_by' => USERID]);
				}
				if($txtStatus == 'P') {
					$article_details['LiveArticleDetails']['url'] 		= $article_details['url'];
					$article_details['LiveArticleDetails']['content_id'] = $article_id;
				...
				$this->db->insert("article_log",["log"=>$articlelogjson,"created_by"=>USERID,"article_id"=>$article_id]);
				if(isset($todayspaper) && $todayspaper=='on'){
					$todaysPaperResponse = $this->live_db->query("SELECT tid FROM todayspaper WHERE content_id='".$article_id."'");
					if($todaysPaperResponse->num_rows > 0){
						$Tid = $todaysPaperResponse->result();
						$this->live_db->where("tid",$Tid[0]->tid);
						$this->live_db->update("todayspaper",["content_id"=>$article_id,"section_name"=>$article_details['LiveArticleDetails']["section_name"],"title"=>trim(addslashes($txtArticleHeadLine)),"url"=>addslashes($article_details["url"]),"publish_start_date"=>trim($article_details['PublishStartDate']),"status"=>$txtStatus,"created_by"=>USERID,"section_id"=>$ddMainSection]);
					}else{
						$this->live_db->insert("todayspaper",["content_id"=>$article_id,"section_name"=>$article_details['LiveArticleDetails']["section_name"],"title"=>trim(addslashes($txtArticleHeadLine)),"url"=>addslashes($article_details["url"]),"publish_start_date"=>trim($article_details['PublishStartDate']),"status"=>$txtStatus,"created_by"=>USERID,"section_id"=>$ddMainSection]);
					...
					$this->live_db->query("DELETE FROM todayspaper WHERE content_id='".$article_id."'");
				}
				if(@$utm_desk_id!=''){
					$CI = &get_instance();
					$this->printdesk_db = $CI->load->database('printdesk_db', TRUE);
	...
	*/
    ...
    default:
		...
		$recordsFiltered = $this->db->query('CALL article_datatable(" ORDER BY '.$order_field.' '.$order.' LIMIT 0, 800 ","'.$check_in.'","'.$check_out.'","'.addslashes($Search_value).'","'.$Search_by.'","'.$Section.'","'.$Status.'")')->num_rows();
		...
		foreach($article_manager as $article) {
			...
			$Firstname=$this->db->query("SELECT Username FROM usermaster WHERE User_id=(SELECT Modifiedby FROM articlemaster WHERE content_id='".$article['content_id']."')")->result();
			...
			if($CheckInYear <= $CurrentYear) {
				...
				if ($this->archive_db->table_exists($TableName)) {
					...
					$ArchiveRecordsFiltered = 0;
							...
							$this->archive_db->select("content_id,title,publish_start_date,status, url,articlepageimageid,homepageimageid,sectionpageimageid,author_name,modified_by,modified_on,created_by");
					...
					if($ArchiveRecordsFiltered != 0 ){
							...
							if($article['articlepageimageid'] != '' ||  $article['homepageimageid'] || $article['sectionpageimageid'] ) {
									if($article['articlepageimageid'] != '') {
										...
										$subdata[] = '<td '.$article['articlepageimageid'].'><a href="javascript:void()"><i class="fa fa-picture-o"></i></a><div class="img-hover"><img  src="'.image_url.imagelibrary_image_path.$Image150X150.'" /></div></td>';
									} else {
	...
	*/
	...
	public function get_seo_details($article_id){
		return $this->live_db->query('SELECT tag_name FROM seo_tags WHERE content_id ='.$article_id.'');
	}
