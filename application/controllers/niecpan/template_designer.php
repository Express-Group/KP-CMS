...
class Template_designer extends CI_Controller {
	...
	public function save_xml_template()
	{
		...
		if($design_lock_status == 2) /* Template is unlocked */
		{
			...
			{
				/*xml backup before publishing */
				if($section_id!=10000 && $section_id!=10001 && $section_id!= 338):
					$backup_parameter=['PageType'=>@$page_type,'menu_id'=>@$section_id];
					//$this->template_design_model->xml_backup($backup_parameter);
				endif;
				/* end */
	...
	public function show_articles_all(){
		print_r($_POST);
	}
	...
	public function save_chart(){
		...
		$widgetinstanceID = $this->input->post('widgetinstanceid');
		$chartname = $this->input->post('chart_name');
		$charttype = $this->input->post('chart_type');
		$clonestatus = $this->input->post('clone_status');
		$total = $this->input->post('total');
		$party_name  = $this->input->post('party_name');
		$party_value  = $this->input->post('party_value');
		$party_color = $this->input->post('party_color');
		$filepath = FCPATH.'application/views/piechart/'.$widgetinstanceID.'.json';
		$data = array('instance_id'=>$widgetinstanceID , 'chartname' => $chartname , 'charttype' => $charttype , 'clonestatus' => $clonestatus , 'total' => $total , 'partyname' =>$party_name , 'partyvalue' => $party_value , 'partycolor'=> $party_color , 'pageid' => $pageid);
		file_put_contents($filepath , json_encode($data));
		chmod($filepath , 0777);
		
		$post_data = array('file_name' => $widgetinstanceID.'.json','file_contents'=> json_encode($data));
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, BASEURL.'user/commonwidget/store_piechart');
		curl_setopt($ch, CURLOPT_POST,1);
		curl_setopt($ch, CURLOPT_POSTFIELDS, $post_data);
		$result=curl_exec ($ch);
		curl_close ($ch);
	...
	public function common_scripts(){
		$homeHeader = trim($this->input->post('home_script'));
		$sectionHeader = trim($this->input->post('section_script'));
		$articleHeader = trim($this->input->post('article_script'));
		$data = ['home_header_script' => addslashes($homeHeader) , 'section_header_script' => addslashes($sectionHeader) , 'article_header_script' =>addslashes($articleHeader)];
		$this->load->database();
		$CI = &get_instance();
		$this->live_db = $CI->load->database('live_db' , true);
		$this->db->where('settings_id', 1);
		$this->db->update('settings' , $data);
		$this->live_db->where('settings_id' ,1);
		echo $this->live_db->update('settings' , $data);
	}
	
	public function get_common_script(){
		$Menu_id = get_menu_details_by_menu_name('Common Header Scripts');
		if(defined("USERACCESS_VIEW".$Menu_id) && constant("USERACCESS_VIEW". $Menu_id) == 1) {
			$this->load->database();
			$record = $this->db->query("SELECT home_header_script , section_header_script , article_header_script FROM settings WHERE settings_id=1")->row_array();
