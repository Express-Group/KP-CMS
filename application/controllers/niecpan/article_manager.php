...

{
...

	{
		...
		if (check_article_id($article_id) != 0)
		{
			...
			$get_seo_detail 			= $this->article_model->get_seo_details($article_id)->row_array();
			$tags 			= $data['get_article_details']['Tags'];
			$agency_id 		= $data['get_article_details']['Agency_ID'];
			$seo_tags 			= @$get_seo_detail['tag_name'];
			if(isset($tags) && trim($tags) != '') 
			//$data['get_tags']				= $this->common_model->get_tags_by_id($tags);
			$data['get_tags']				= $this->common_model->get_tags_by_order($tags);
			if(isset($seo_tags) && trim($seo_tags) != '') 
			$data['seo_tags'] = explode(',' , $seo_tags); 
			...
			*/
			...
			$data['relatedimagedata'] = $this->article_model->getrelatedimagedata($article_id);
			if($this->input->post('utm')=='printdesk'){
				$data['template'] 			= 'agencies_article';
			}
			$data['todayspaper'] = $this->article_model->gettodayspaperstatus($article_id);
			//added to check seo admin type
			 $UserAccess=$this->article_model->useraccess($this->session->userdata('userID'));
			if($UserAccess=='seoadmin'):
				$data['template'] 				= 'seo_viewsarticle';
			endif; 
			//end..
		...
		if($contentId!=''){
			...
			if($checkId==0){
				...
				if(count($details) > 0){
					switch($type){
						case 'koo' :
							$headers = ['Content-Type:application/json' , 'X-KOO-API-TOKEN:2f8894ab-2a6c-46ac-9509-1eda2a002538'];
