		...
		$module		= '';
		   ...
		   switch($content_name) {
			...
			case "general_report": 
			 ...
			 break;
			  case "add_widget": 
						 $module 		= "view widget master page";					 	 
			 break;
			 case "amp_advertisements": 
						 $module 		= "AMP Advertisements";					 	 
			 break;
		   }
...
class byline_autocomlete extends common
{
	function get_author_name()
	{
		...
		foreach ($get_byline_name as $key=>$row){
			$type = ($row['authorType']=='1') ? ' (byline)' : ' (columnist)';
			$data[$key]['label'] = $row['AuthorName'].$type;
		...
		try
		{
			...
			if (isset($ImageDetails['image_name']))
			{
				$image_src_path= '';
					...
					case "image_900_450":
							$dst 		= $image_src_path. str_replace(".","_900_450.",$ImageDetails['image_name']);
							$source = $source.str_replace(".","_900_450.",$ImageDetails['image_name']);
					break;
				...
				if (!empty($src) && !empty($dst) && !empty($data))
				{
					...
					break;
				 	...
				 	case "image_900_450":
							update_temp_images( $content_id, $crop_caption, $crop_alt,$Null_value,$Null_value,$Null_value,2, $modifiedon, $crop_image_id,'image_900_450',0,1);
